...
def is_valid_email(email):
    pattern = r"^[a-z]+(?:\d{2,})?@[a-z]+\.(com|co\.id)$" # TODO: Buatkan pola RegEx untuk validasi email.
...
if is_valid_username(username):
    ...
    if is_valid_email(email):
        ...
        if is_valid_password(password):
            print(f"\nRegistrasi berhasil! Selamat datang, {username} ") # TODO: Tambahkan sapaan "Halo (username)"
        else:
