...
6. Keluar""")
    ...
    # utk Opsi 3 (Jumlah Data pada File)
    ...
    # utk Opsi 4 (Save Data pada File)
    def save_data(self):
        if not any(self.data.values()):
            print(self.garis + "\nData saat ini kosong")
        else:
...
{self.garis}"""
    ...
    # utk Opsi 5
    def validasi_email(self, email):    # Pola utk Mengvalidasi Email
        pattern = r'^[a-z0-9]+@student\.unhas\.ac\.id$'
        return re.search(pattern, email)

    def validasi_password(self, password):  # Pola utk Mengvalidasi Password
        pattern = r"^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)[A-Za-z\d]{8,}$"
        return re.search(pattern, password)

    # utk Membuat Data Baru
    def buat_data(self):
        print(self.garis)
